...
export default function Services() {
  ...
  return (
    <div className="pt-20">
      {/* Hero */}
      <section className="bg-[#0A2540] py-40 text-white text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            {serviceList.map((service, index) => (
              <motion.div
                ...
                className="bg-white p-12 rounded-[40px] shadow-sm border border-gray-100 flex flex-col justify-between hover:shadow-2xl transition-all group"
              >
                <div>
                  <div className="bg-[#0A2540]/5 w-20 h-20 rounded-3xl flex items-center justify-center text-[#E85D04] mb-8 group-hover:bg-[#E85D04] group-hover:text-white transition-colors">
                    {service.icon}
                  </div>
                  <div className="mb-8">
                    <span className="text-xs font-bold uppercase tracking-widest text-[#E85D04] mb-2 block">{service.tagline}</span>
                    <h2 className="text-3xl font-black text-[#0A2540] tracking-tighter">{service.title}</h2>
                  </div>
                  ...
                  <ul className="space-y-4 mb-12">
                    {service.features.map((f, i) => (
                      <li key={i} className="flex items-center text-sm font-medium text-gray-700">
                        <CheckCircle2 className="w-5 h-5 text-[#E85D04] mr-3 shrink-0" />
                        {f}
                      </li>
                    ))}
                  </ul>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            ...
            <div className="order-1 lg:order-2">
              <h2 className="text-4xl md:text-5xl font-black text-[#0A2540] mb-8 tracking-tighter">Why Choose USAL?</h2>
              <div className="space-y-8">
                <div className="flex items-start space-x-6">
                  <div className="text-[#E85D04] font-bold text-4xl">01</div>
                  <div>
                    <h4 className="font-bold text-xl mb-2 text-[#0A2540]">100% Visibility</h4>
                    <p className="text-gray-600 text-sm">Full ELD integration across our entire fleet provides real-time GPS tracking for every vehicle.</p>
                  </div>
                </div>
                <div className="flex items-start space-x-6">
                  <div className="text-[#E85D04] font-bold text-4xl">02</div>
                  <div>
                    <h4 className="font-bold text-xl mb-2 text-[#0A2540]">Superior Equipment</h4>
                    <p className="text-gray-600 text-sm">We maintain a 10/10 equipment quality rating, ensuring damage-free delivery every time.</p>
                  </div>
                </div>
                <div className="flex items-start space-x-6">
                  <div className="text-[#E85D04] font-bold text-4xl">03</div>
                  <div>
                    <h4 className="font-bold text-xl mb-2 text-[#0A2540]">Expert Drivers</h4>
                    <p className="text-gray-600 text-sm">Our 392 drivers are among the most qualified in the industry with rigorous safety training.</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      {/* Final CTA */}
      <section className="py-24 bg-[#E85D04] text-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-4xl md:text-6xl font-black mb-8 tracking-tighter">Custom Logistical Planning</h2>
          <p className="text-xl mb-12 opacity-90">Our expert team can design a custom logistics workflow for your specific national or regional needs.</p>
          <Link to="/contact" className="inline-block px-12 py-5 bg-[#0A2540] text-white rounded-full font-bold hover:bg-gray-900 transition-all text-lg">
